...
{
	// 首次触发时枚举目录下的图片
	if (noHardwareImages2.isEmpty())
	{
		const QDir dir(globalPath.testImgRootPath);
		noHardwareImages2 = dir.entryList(
			{ "*.jpg", "*.jpeg", "*.png", "*.bmp" }, QDir::Files, QDir::Name);
		noHardwareImageIndex2 = 0;
		if (noHardwareImages2.isEmpty())
		{
			qWarning("NoHardware: no image found in %s", qPrintable(globalPath.testImgRootPath));
			return;
	...
	const QString imagePath = globalPath.testImgRootPath + noHardwareImages2.at(noHardwareImageIndex2);
	// 循环播放目录中的图片
	noHardwareImageIndex2 = (noHardwareImageIndex2 + 1) % noHardwareImages2.size();
